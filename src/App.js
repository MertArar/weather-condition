...
const App = () => {
  ...
  const [location, setLocation] = useState("Konya");
  ...
  return (
    <div className="w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col items-center justify-center px-4 lg:px-0">
      {errorMsg && (
        <div className="w-full max-w-[90vw] lg:max-w-[450px] bg-[#ff2020] text-white absolute top-2 lg:top-10 p-4 capitalize rounded-md">{`${errorMsg.response.data.message}`}</div>
      )}
      {/* form */}
      <motion.form
      ...
      <motion.div
        ...
        whileInView={"show"}
        viewport={{ once: false, amount: 0.3 }}
        className="text-white/70 flex text-center mt-20"
      >
        Developed by Mert Arar
      </motion.div>
    </div>
  );
};
